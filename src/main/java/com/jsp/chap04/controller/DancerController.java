...
// 응답할 view를 결정합니다.
@WebServlet("*.do") // 확장자 패턴 -> 앞이 뮈든간에 .do로 끝나는 요청이라면 이 서블릿이 다 받겠다.
public class DancerController extends HttpServlet {

   //인터페이스 타입의 변수를 선언해서, 상황에 맞게 서비스 객체를 같아 끼을 겁니다.
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      try {
         switch (requestURI) {
            case "/register.do":
               System.out.println("등록 품으로 이동시켜 달라는 요청이구나!");
               // User getRequestDispatcher to be able to go inside the pathway inside web-inf
            ...
            case "/regist.do":
               ...
               // Dispatcher에게 목적지를 알려주면서, request와 response객체를 가지고 이동하라는 명령을 내립니다.
